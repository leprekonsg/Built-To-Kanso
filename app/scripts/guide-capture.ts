...
async function main(): Promise<void> {
  ...
  try {
    ...
    // 06 — Bones hero. Click Continue and wait for the next route.
    await Promise.all([
      page.waitForURL(/\/bones\?/),
    ...
    // 07 — Asking points (Damp Risk, Anti-cure).
    await page.getByText("What the home is asking", { exact: true }).scrollIntoViewIfNeeded();
    await shot(page, "07-bones-asking-points");

    // 08 — LiveStudio (airflow visual region).
    await page.getByRole("region", { name: "LiveStudio" }).scrollIntoViewIfNeeded();
    ...
    // 11 — Three action cards.
    await page.getByRole("heading", { name: /Three decisions, one receipt\./ }).scrollIntoViewIfNeeded();
    await shot(page, "11-proof-actions");

    // 12 — House Changelog + sketch comparison.
    await page.getByText("House Changelog").scrollIntoViewIfNeeded();
    await shot(page, "12-proof-changelog");
    ...
    // 16 — Evidence ladder section.
    await page.locator("#evidence").scrollIntoViewIfNeeded();
    await shot(page, "16-methodology-evidence");
  } finally {
    await browser.close();
  }
}
