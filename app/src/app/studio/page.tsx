...
function ParamErrorsState({ issues }: { issues: ThresholdParamIssue[] }) {
  return (
    <main className={styles.missingPage} data-testid="studio-param-errors">
      <header className={styles.masthead}>
        <Link href="/threshold" className={styles.brand} aria-label="Built-To-Kanso, return to Threshold">
          <span className={styles.brandMark}>
            Built<span className={styles.brandHyphen}>-</span>To<span className={styles.brandHyphen}>-</span>Kanso
          </span>
          <span className={styles.brandSub}>Studio · Anteroom</span>
        </Link>
      </header>
      ...
      <section className={styles.missingHero}>
        ...
        <p className={styles.missingLede}>
          {issues.length === 1
            ? "One Threshold input didn’t match what the Studio expects."
            : `${issues.length} Threshold inputs didn’t match what the Studio expects.`}{" "}
          Either fix the URL using the rows below, or set the inputs again at the Threshold.
        </p>
        <dl className={styles.issueList} data-testid="studio-param-issues">
          {issues.map((issue) => (
...
function MissingInputs() {
  return (
    <main className={styles.missingPage}>
      ...
      <section className={styles.missingHero}>
        <span className={styles.eyebrow}>Studio · Anteroom</span>
        <h1 className={styles.missingHeadline}>
          The house has not yet<br />
          <em>heard you</em>.
        </h1>
        <p className={styles.missingLede}>
          The Studio reads a unit&rsquo;s plan after the threshold lands. It cannot listen until
          it knows which home to listen to. Choose a template, set the door and floor; ninety
          seconds will do it.
        </p>
        <Link href="/threshold" className={styles.missingPrimary}>
      ...
      <section className={styles.missingMethod} aria-labelledby="studio-anteroom-method">
        <svg
        ...
        <span id="studio-anteroom-method" className={styles.eyebrow}>Method</span>
      ...
      <section className={styles.missingPreview} aria-label="What the Studio will read">
        <span className={styles.eyebrow}>Preview · the Studio will read</span>
